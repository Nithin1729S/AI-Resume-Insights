  ...
  return (
    <div className="dark:bg-gray-dark rounded-[10px] bg-white px-7.5 pb-4 pt-7.5 shadow-1 dark:shadow-card">
      ...
      <div className="flex flex-col">
        {/* Header Row */}
        <div className="dark:border-dark-3 grid grid-cols-7 border-b border-stroke">
        ...
        {resumeData.map((RESUME, key) => (
          <div
            className={`grid grid-cols-7 ${
              key === resumeData.length - 1
                ? ""
                : "dark:border-dark-3 border-b border-stroke"
            }`}
            key={key}
          >
            {/* Resume Image */}
            ...
            {/* Style */}
            <div className="flex items-center justify-center px-2 py-4">
              <p className="text-green-light-1 font-medium">{RESUME.style}</p>
            </div>
...
const downloadPDF = (pdfUrl: string): void => {
  fetch(pdfUrl)
    ...
    .then((blob) => {
      ...
      link.setAttribute("download", pdfUrl.split("/").pop() || "download.pdf");
